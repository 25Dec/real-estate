...
<template>
  ...
  >
    ...
    <template class="flex flex-col gap-3">
      <div class="flex gap-3">
        <div class="flex flex-1 flex-col gap-2">
          ...
          <InputText id="id" v-model="id" disabled />
        </div>

        <div class="flex flex-1 flex-col gap-2">
          <label for="zone_id">Zone ID</label>
          <InputText id="zone_id" v-model="zoneID" disabled />
        </div>
      </div>

      <div class="flex flex-1 flex-col gap-2">
        <label for="desc">Name</label>
        <InputText id="desc" v-model="desc" disabled />
      </div>

      <div class="flex flex-1 flex-col gap-2">
        <label for="zone">Zone</label>
        <InputText id="zone" v-model="currentZone['name']" disabled />
      </div>

      <div class="flex gap-3">
        <div class="flex flex-1 flex-col gap-2">
          <label for="lat">Latitude</label>
          <InputNumber id="lat" v-model="lat" disabled />
        </div>
        <div class="flex flex-1 flex-col gap-2">
          <label for="long">Longitude</label>
          <InputNumber id="long" v-model="long" disabled />
        </div>
      </div>

      <div class="flex flex-1 flex-col gap-2">
        <label for="type">Type</label>
        <InputText id="type" v-model="type" disabled />
      </div>

      <div class="flex flex-1 flex-col gap-2">
        <label for="numberOfFloor">Number of floor</label>
        <InputNumber id="numberOfFloor" v-model="numberOfFloor" disabled />
      </div>
      ...
      <div class="flex flex-1 flex-col gap-2 items-end">
        ...
        <InputSwitch id="isService" v-model="isService" disabled />
      </div>
    ...
    <template #footer>
      <Button type="button" label="Close" @click="myVisible = false" />
    </template>
  </Dialog>
</template>

  ...
  <div class="w-1/6 h-screen">
    ...
    >
      <div class="w-full h-fit flex flex-col gap-1">
        <SidebarItem v-for="link in links" :key="link.name" :data="link" />
      </div>
    </div>
    ...
    >
      <div v-if="Object.keys(user).length" class="flex items-center gap-2">
        <Avatar :label="user['display_name'].substring(0, 3)" shape="square" />
        <span class="font-semibold">{{ user["display_name"] }}</span>
      </div>
      <Button @click="onLogout" text plain>

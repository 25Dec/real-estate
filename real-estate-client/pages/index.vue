...
<template>
  ...
  >
    <NuxtLink to="/" class="flex justify-center items-center gap-1">
      <Icon name="line-md:home-simple" class="text-2xl text-[#10b981]" />
    ...
    >
      ...
      <li
        class="hover:scale-150 hover:-translate-x-6 transition duration-500 ease-in-out"
      >
        <NuxtLink :to="{ hash: '#about' }" :external="true"> About </NuxtLink>
      </li>
      <li
        class="hover:scale-150 hover:-translate-x-6 transition duration-500 ease-in-out"
      >
        <NuxtLink :to="{ hash: '#services' }" :external="true">
          Services
        </NuxtLink>
      </li>
      <li
        class="hover:scale-150 hover:-translate-x-6 transition duration-500 ease-in-out"
      >
        <NuxtLink :to="{ hash: '#projects' }" :external="true">
          Projects
        </NuxtLink>
      </li>
      <!-- <li -->
      ...
      <li
        class="hover:scale-150 hover:-translate-x-6 transition duration-500 ease-in-out"
      >
        <NuxtLink :to="{ hash: '#contact' }" :external="true">
          Contact
        </NuxtLink>
      </li>
    </ul>
  ...
  >
    <div v-motion-slide-visible-left class="flex justify-end items-center">
      <NuxtImg src="/images/about_banner.png" class="shadow-2xl rounded-lg" />
    </div>
    <div v-motion-slide-visible-right class="flex flex-col gap-6 w-4/5">
      <span class="text-4xl w-fit font-bold p-2 border-b">About Us</span>
      <span class="text-2xl font-semibold"
        >Provide service to manage real estate.</span
      >
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <AboutCard v-for="about in abouts" :key="about.title" :data="about" />
      </div>
      <NuxtLink :to="{ hash: '#services' }" :external="true">
        <Button size="large">
  ...
  >
    ...
    <div v-motion-pop-visible class="flex flex-col w-1/2 gap-4">
      <div class="flex flex-1 flex-col gap-2">
        <label for="fullName">Full Name:</label>
        <InputText id="fullName" size="large" />
      </div>
      <div class="flex flex-1 flex-col gap-2">
        <label for="email">Email:</label>
        <InputText id="email" size="large" />
      </div>
      <div class="flex flex-1 flex-col gap-2">
        <label for="email">Phone Number:</label>
        <InputText id="phoneNumber" size="large" />
      </div>
      <div class="flex flex-1 flex-col gap-2">
      ...
      <div class="flex flex-1">
        <Button class="flex-1" label="Submit" />
      </div>
    </div>
  </section>

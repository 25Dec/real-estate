...
const handleDropdown = (event) => {
  myPaymentMethodsProcessBaseOnPMId.value = paymentMethodsProcess.value.filter(
    (process) => {
...
<template>
  <div class="w-full h-full flex flex-col">
    ...
    >
      <div class="flex items-center gap-2">
        <label for="currentPaymentMethod" class="font-semibold text-lg"
    ...
    <div class="absolute top-[16%] w-full h-[92%]">
      ...
      >
        ...
        <Column field="include_vat" header="Include VAT">
          <template #body="{ data }">
            {{ data["include_vat"] == true ? "Yes" : "No" }}
          </template>
        </Column>

        <Column field="total_percent_payment" header="Total payment" sortable>
          <template #body="{ data }">
            <Knob v-model="data['total_percent_payment']" readonly :size="50" />
          </template>
        </Column>
